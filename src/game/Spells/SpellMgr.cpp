...
void SpellMgr::LoadSpellChains()
{
    ...
    // load custom case
    QueryResult *result = WorldDatabase.PQuery("SELECT spell_id, prev_spell, first_spell, `rank`, req_spell FROM spell_chain WHERE (build_min <= %u) AND (build_max >= %u)", SUPPORTED_CLIENT_BUILD, SUPPORTED_CLIENT_BUILD);
    if (!result)
    {
        BarGoLink bar(1);
        bar.step();
